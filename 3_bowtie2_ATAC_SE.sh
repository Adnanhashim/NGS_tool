...
# Project: test
#SBATCH --job-name=ATAC_SE_ATAC
#SBATCH --time=8:00:00
#SBATCH --mem-per-cpu=3G
#SBATCH --cpus-per-task=20
#SBATCH --output=/cluster/projects/nn8009k/Adnan/000_scripts/temp_slurm_output/3_bowtie2_SE_ATAC_%j.log
THREADS=20
...
ERR=$OUTDIR/$out.bowtie2_ATAC_SE.err
...
date
    if [ ! -e "$CHECK2" ] ; then
        ...
        if bowtie2 -p $THREADS -x $index --very-sensitive -U $fq1 -S $USERWORK/${out}.sam 2> $ERR
           ...
           samtools markdup -@ $THREADS -r -s ${out}_sorted.bam $OUTDIR/${out}_dedup.bam
           samtools index $OUTDIR/${out}_dedup.bam $OUTDIR/${out}_dedup.bam.bai
           
           echo "creating bigwig"
           #bamCoverage -p $THREADS --bam $OUTDIR/${out}_dedup.bam -o $OUTDIR/${out}_dedup_RPGC.bw --binSize 10 --normalizeUsing RPGC --effectiveGenomeSize $effective_genome_size --ignoreForNormalization chrX chrM --ignoreDuplicates
